...
function init(){
	...
	var MAP_RAW_JSON_TO_GRAPH = function(_ROOTS_){
		...
		$.each(Object.keys(_ROOTS_), function(index, root){
			...
			var rootIndex = -1;
			rootIndex = -1 + _MAP_.children[letterIndex].children.push( { "id": root, "name": root, data: {}, children: [] } );
			
			//3) Now add children of the root
			var rootChildren = _ROOTS_[ root ].d;
			if(rootChildren)
				$.each( Object.keys(rootChildren), function(childno, child){
					_MAP_.children[letterIndex].children[ rootIndex ].children.push( { "id": child, "name": child, data: {}, children: [] } );
				});
		});
		return _MAP_;
	}
    ...
    var st = new $jit.ST({
        ...
        onCreateLabel: function(label, node){
			...
			if(url){
				label.innerHTML = "&nbsp;" + label.innerHTML + "&nbsp;<span dir=ltr style=font-size:0.3em;><A HREF='" + url + "' TARGET=_>(link)" + "</A></span>";
			}
